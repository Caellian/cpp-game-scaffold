...
static std::array<const char *, 9>
  allocationTypes = { "Unknown", "new", "new[]", "malloc", "calloc", "realloc", "delete", "delete[]", "free" };
const char *allocation_name(alloc_type ty) { return allocationTypes[static_cast<unsigned int>(ty)]; }
...
static sAllocUnit *findAllocUnit(const void *reportedAddress)
{
  ...
  // allocated addresses will be on four-, eight- or even sixteen-byte boundaries. If we didn't do this, the hash index
  // would not have very good coverage.

  uintptr_t hashIndex = (reinterpret_cast<uintptr_t>(const_cast<void *>(reportedAddress)) >> 4) & (hashSize - 1);
...
static void wipeWithPattern(sAllocUnit *allocUnit, unsigned long pattern, const unsigned int originalReportedSize = 0)
{
  ...
  long *post = reinterpret_cast<long *>(
    reinterpret_cast<char *>(allocUnit->actualAddress) + allocUnit->actualSize - paddingSize * sizeof(long));
...
{
  fprintf(
    fp, "Alloc.   Addr       Size       Addr       Size                        BreakOn BreakOn              \r\n");
  fprintf(
    fp, "Number Reported   Reported    Actual     Actual     Unused    Method  Dealloc Realloc Allocated by \r\n");
  fprintf(fp,
    "------ ---------- ---------- ---------- ---------- ---------- -------- ------- ------- "
    "--------------------------------------------------- \r\n");
  ...
  for (uintptr_t i = 0; i < hashSize; i++) {
    ...
    while (ptr) {
      fprintf(fp,
        ...
        reinterpret_cast<uintptr_t>(ptr->reportedAddress),
        ptr->reportedSize,
        reinterpret_cast<uintptr_t>(ptr->actualAddress),
        ptr->actualSize,
        m_calcUnused(ptr),
        allocation_name(ptr->allocationType),
...
static void dumpLeakReport()
{
  ...
  if (stats.totalAllocUnitCount) {
    ...
    if (reservoirBuffer) {
      for (unsigned int i = 0; i < reservoirBufferSize; i++) { free(reservoirBuffer[i]); }
...
bool &m_breakOnRealloc(void *reportedAddress)
{
  ...
  // If you hit this assert, you tried to set a breakpoint on reallocation for an address that wasn't allocated in a way
  // that is compatible with reallocation.
  m_assert(au->allocationType == alloc_type::m_malloc || au->allocationType == alloc_type::m_calloc
           || au->allocationType == alloc_type::m_realloc);
...
void m_setOwner(const char *file, const size_t line, const char *func)
{
  ...
  // This is fine... usually. Here's what actually happens when you use this macro to delete an object:
  //
  ...
  // delete. But what happens when we delete an object whose destructor deletes another object, whose desctuctor deletes
  // another object? Here's a diagram (indentation follows stack depth):
  //
  ...
  // chain will not have ANY owner-reporting information. The deletes will still work fine, we just won't know who
  // called us.
  //
  ...
  // those cases, m_setOwner() is never called, and rightfully should not have the proper trace-back information. So if
  // one of the destructors in the chain ends up being a call to a delete from a non-mmgr-compiled library, the stack
  // will get confused.
  //
...
void *operator new(size_t reportedSize)
{
  ...
  for (;;) {
    ...
    void *ptr = m_allocator(file, line, func, alloc_type::m_new, reportedSize);
    if (ptr) {
#ifdef TEST_MEMORY_MANAGER
      log("[D] EXIT : new");
#endif
...
void *operator new[](size_t reportedSize)
{
  ...
  for (;;) {
    ...
    void *ptr = m_allocator(file, line, func, alloc_type::m_new_array, reportedSize);
    if (ptr) {
#ifdef TEST_MEMORY_MANAGER
      log("[D] EXIT : new[]");
#endif
...
// our memory tracking efforts. Like the previous versions, these are merely interface functions that operate like
// normal new/new[], but use our memory tracking routines.
// ---------------------------------------------------------------------------------------------------------------------------------
...
void *operator new(size_t reportedSize, const char *sourceFile, size_t sourceLine)
{
  ...
  for (;;) {
    ...
    void *ptr = m_allocator(sourceFile, sourceLine, "??", alloc_type::m_new, reportedSize);
    if (ptr) {
#ifdef TEST_MEMORY_MANAGER
      log("[D] EXIT : new");
#endif
...
void *operator new[](size_t reportedSize, const char *sourceFile, size_t sourceLine)
{
  ...
  for (;;) {
    ...
    void *ptr = m_allocator(sourceFile, sourceLine, "??", alloc_type::m_new_array, reportedSize);
    if (ptr) {
#ifdef TEST_MEMORY_MANAGER
      log("[D] EXIT : new[]");
#endif
...
// delete/delete[], but use our memory tracking routines.
// ---------------------------------------------------------------------------------------------------------------------------------
...
void operator delete(void *reportedAddress)
{
  ...
  if (reportedAddress)
    m_deallocator(sourceFile, sourceLine, sourceFunc, alloc_type::m_delete, reportedAddress);
  else if (alwaysLogAll)
    log("[-] ----- %8s of NULL                      by %s",
      allocation_name(alloc_type::m_delete),
...
void operator delete[](void *reportedAddress)
{
  ...
  if (reportedAddress)
    m_deallocator(sourceFile, sourceLine, sourceFunc, alloc_type::m_delete_array, reportedAddress);
  else if (alwaysLogAll)
    log("[-] ----- %8s of NULL                      by %s",
      allocation_name(alloc_type::m_delete_array),
      ownerString(sourceFile, sourceLine, sourceFunc));
  ...
  try {
    ...
    if (alwaysLogAll)
      log("[+] %05d %8s of size 0x%08X(%08d) by %s",
        currentAllocationCount,
        allocation_name(allocationType),
        reportedSize,
        reportedSize,
        ownerString(sourceFile, sourceLine, sourceFunc));
    ...
    if (au->actualAddress == NULL) { throw "Request for allocation failed. Out of memory."; }
    ...
    m_assert(allocationType != alloc_type::m_unknown);

    // Insert the new allocation into the hash table

    uintptr_t hashIndex = (reinterpret_cast<uintptr_t>(au->reportedAddress) >> 4) & (hashSize - 1);
    ...
    stats.totalReportedMemory += static_cast<uintptr_t>(au->reportedSize);
    stats.totalActualMemory += static_cast<uintptr_t>(au->actualSize);
    ...
    stats.accumulatedReportedMemory += static_cast<uintptr_t>(au->reportedSize);
    stats.accumulatedActualMemory += static_cast<uintptr_t>(au->actualSize);
    ...
    if (allocationType == alloc_type::m_calloc) { memset(au->reportedAddress, 0, au->reportedSize); }
    ...
    if (alwaysLogAll) log("[+] ---->             addr 0x%08X", reinterpret_cast<uintptr_t>(au->reportedAddress));
  ...
  void *reportedAddress)
{
  try {
    ...
    if (!reportedAddress) { return m_allocator(sourceFile, sourceLine, sourceFunc, reallocationType, reportedSize); }
    ...
    if (alwaysLogAll)
      log("[~] %05d %8s of size 0x%08X(%08d) by %s",
        currentAllocationCount,
        allocation_name(reallocationType),
        reportedSize,
        reportedSize,
        ownerString(sourceFile, sourceLine, sourceFunc));
    ...
    m_assert(reallocationType != alloc_type::m_unknown);
    ...
    uintptr_t hashIndex = static_cast<uintptr_t>(-1);
    if (oldReportedAddress != au->reportedAddress) {
      // Remove this allocation unit from the hash table

      {
        uintptr_t hashIndex = (reinterpret_cast<uintptr_t>(oldReportedAddress) >> 4) & (hashSize - 1);
      ...
      hashIndex = (reinterpret_cast<uintptr_t>(au->reportedAddress) >> 4) & (hashSize - 1);
      if (hashTable[hashIndex]) hashTable[hashIndex]->prev = au;
      au->next = hashTable[hashIndex];
      au->prev = NULL;
      hashTable[hashIndex] = au;
  ...
  try {
    ...
    if (alwaysLogAll)
      log("[-] ----- %8s of addr 0x%08X           by %s",
        allocation_name(deallocationType),
        reinterpret_cast<uintptr_t>(const_cast<void *>(reportedAddress)),
        ownerString(sourceFile, sourceLine, sourceFunc));

    // We should only ever get here with a null pointer if they try to do so with a call to free() (delete[] and delete
    // will both bail before they get here.) So, since ANSI allows free(NULL), we'll not bother trying to actually free
    // the allocated memory or track it any further.
    ...
    if (reportedAddress) {
      ...
      // tracking software, use the stack frame to locate the source and include our H file.
      m_assert(deallocationType != alloc_type::m_unknown);

      // If you hit this assert, you were trying to deallocate RAM that was not allocated in a way that is compatible
      // with the deallocation method requested. In other words, you have a allocation/deallocation mismatch.
      m_assert((deallocationType == alloc_type::m_delete && au->allocationType == alloc_type::m_new)
               || (deallocationType == alloc_type::m_delete_array && au->allocationType == alloc_type::m_new_array)
               || (deallocationType == alloc_type::m_free && au->allocationType == alloc_type::m_malloc)
               || (deallocationType == alloc_type::m_free && au->allocationType == alloc_type::m_calloc)
               || (deallocationType == alloc_type::m_free && au->allocationType == alloc_type::m_realloc)
               || (deallocationType == alloc_type::m_unknown));
...
bool m_validateAllocUnit(const sAllocUnit *allocUnit)
{
  ...
  for (uintptr_t i = 0; i < paddingSize; i++, pre++, post++) {
...
bool m_validateAllAllocUnits()
{
  ...
  // the offending code. After running the application with these settings (and hitting this assert again), interrogate
  // the memory.log file to find the previous successful operation. The corruption will have occurred between that point
  // and this assertion.
  m_assert(allocCount == stats.totalAllocUnitCount);

  // If you hit this assert, then you've probably already been notified that there was a problem with a allocation unit
  // in a prior call to validateAllocUnit(), but this assert is here just to make sure you know about it. :)
  m_assert(errors == 0);
...
{
  const uintptr_t *ptr = reinterpret_cast<const uintptr_t *>(allocUnit->reportedAddress);
  size_t count = 0;

  for (uintptr_t i = 0; i < allocUnit->reportedSize; i += sizeof(long), ptr++) {
...
void m_dumpAllocUnit(const sAllocUnit *allocUnit, const char *prefix)
{
  ...
  log("[I] %sAllocation type   : %s", prefix, allocation_name(allocUnit->allocationType));
